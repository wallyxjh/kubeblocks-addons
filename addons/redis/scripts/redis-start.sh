...
build_redis_info_replication_command() {
  ...
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    echo "timeout $timeout_value redis-cli $REDIS_CLI_TLS_CMD -h $redis_pod_fqdn -p $service_port INFO replication"
  else
    echo "timeout $timeout_value redis-cli $REDIS_CLI_TLS_CMD -h $redis_pod_fqdn -p $service_port -a $REDIS_DEFAULT_PASSWORD INFO replication"
  fi
}
